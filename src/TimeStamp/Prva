...
class Scheduler <T>
{
    ...
    public boolean remove(Timestamp<T>t) {
        ...
        {
            timestampList.remove(t);
            return !timestampList.contains(t);
        }
        else return false;
    }
    public Timestamp<T> next()
    {
        return timestampList.stream().filter(Timestamp::isAfterNow).sorted().findFirst().orElse(null);
    }
    public Timestamp<T> last() {
        return timestampList.stream()
                .filter(Timestamp::isBeforeNow)
                .sorted(Comparator.<Timestamp, LocalDateTime>comparing(timestamp -> LocalDateTime.from(timestamp.time)).reversed())
                .findFirst()
                .orElse(null);
    }

    public List<Timestamp<T>> getAll(LocalDateTime localDateTime, LocalDateTime localDateTime1) {
    return  timestampList.stream().filter(i->i.time.isAfter(localDateTime) && i.time.isBefore(localDateTime1)).collect(Collectors.toList());
    }
}
