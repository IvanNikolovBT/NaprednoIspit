...
class User {
    ...
    public int getSum() {
    return orders.stream().mapToInt(Double::intValue).sum();
    }
}
...
//        u.orderFood(costToPay+cost);
    ...
    public DeliveryPerson findClosestDeliveryPerson(Restaurant r) {
        return idDelivery.values().stream().map(i -> i.addForSort(r.location)).min(Comparator.comparing(DeliveryPerson::getDistance).thenComparing(DeliveryPerson::getNumberOfOders).thenComparing(DeliveryPerson::getID)).orElse(null);

    }

    public void printUsers() {
        idUser.values().stream().sorted(Comparator.comparing(User::getSum,Comparator.reverseOrder()).thenComparing(User::getAvg, Comparator.reverseOrder()).thenComparing(User::getID,Comparator.reverseOrder())).forEach(System.out::println);
    }
