...
class Rule<IN, OUT> {
    ...
    public Optional<OUT> apply(IN in) {
        if (condition.test(in)) return Optional.of(mapper.apply(in));
    ...
    public static <IN, OUT> void process(List<IN> inputs, List<Rule<IN, OUT>> rules) {
        inputs.forEach(input->
        {
            ...
            rules.forEach( rule-> {
                ...
                if(result.isEmpty())
                {
                    System.out.print("Conditions not met");
                }
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            rules.add(new Rule<>
                    (
                            str->str.contains("NP"),str->str.indexOf("NP")
                    ));
            ...
            * */
            rules.add(new Rule<>
                    (
                            str->str.startsWith("NP"),str->str.length()
                    ));
            ...
            rules.add(new Rule< >
                    (
                    student->student.grades.size()>=3,student->(double)student.grades.stream().mapToInt(i->i).max().getAsInt()
            ...
            rules.add(new Rule<>(
                    student->student.id.startsWith("21"),student->student.grades.stream().mapToDouble(i->i).average().orElse(5.0)
